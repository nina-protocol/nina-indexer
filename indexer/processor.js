...
class NinaProcessor {
  ...
  async processHubs() {
    ...
    for await (let existingHub of existingHubs) {
      ...
      await this.updateHub(existingHub, hubAccount, hubContent, hubReleases, hubCollaborators, hubPosts);
    }
    ...
    for await (let newHub of newHubs) {
      try {
        ...
        await this.updateHub(hub, newHub, hubContent, hubReleases, hubCollaborators, hubPosts);
      } catch (err) {
        console.log(err);
      }
    }
