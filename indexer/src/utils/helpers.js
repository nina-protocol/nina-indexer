...
export const fetchFromArweave = async (uri) => {
  let response;
  try {
    response = await axios.get(ensureHttps(uri.replace('www.', '').replace('arweave.net', 'gateway.irys.xyz'))).then(response => response.data);
  } catch (error) {
    response = await axios.get(ensureHttps(uri.replace('gateway.irys.xyz', 'arweave.net'))).then(response => response.data);
  }
  return response;
}
